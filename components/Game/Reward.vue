...
type Emits = {
  select: [reward: GameElement | Relic | Powerup];
};

const store = useGameStore();
const props = defineProps<Props>();
const emit = defineEmits<Emits>();
const rewards = ref<(GameElement | Relic | Powerup)[]>([]);
const selectedReward = ref<GameElement | Relic | Powerup>();
...
function select(reward: GameElement | Relic | Powerup) {
  selectedReward.value = reward;
  emit("select", reward);
}

function getNewReward(type: "element" | "other") {
  let reward: GameElement | Relic | Powerup | undefined;
  let tries = 0;
  while (!reward && tries < 100) {
    tries++;
    const newReward = type == "element" ? (getRandomItemFromArray([air, earth, fire, ice]) as GameElement) : (getRandomItemFromArray([relics, powerups].flat()) as Relic | Powerup);
