  ...
  <div class="flex items-center justify-center w-full">
    ...
    <div class="relative flex items-center justify-center w-screen h-screen overflow-hidden select-none" :class="{ 'bg-black': level }">
      ...
      <Transition name="pageOther">
        <GameMap :current-level="previousLevel" @select="(lvl) => (level = lvl)" v-if="level == 'map'" />
      </Transition>
    </div>
  </div>
</template>
...
const previousLevel = ref<Level>();
const level = ref<"map" | Level>();
...
const elements = ref<Record<string, GameElement>>({ ice, fire, air, earth });
const selectedElement = ref<GameElement>();
...
function handleReward(reward: GameElement | Relic | Powerup | { type: "Bypass" }) {
...
function getBarColor(element: GameElement, bar: number) {
