<template>
  <div class="flex flex-col items-center justify-start gap-4 w-screen h-screen">
    ...
    <div class="flex items-center justify-center gap-4">
      <NuxtLink to="/" class="back transition px-10 py-2.5 rounded-full border-2 border-[color:var(--text-color)] bg-transparent text-[color:var(--text-color)] text-lg font-semibold mt-6"
        >Back</NuxtLink
      >
      <button
        @click="join(code)"
...
onBeforeMount(() => {
  showBanner.value = JSON.parse(sessionStorage.getItem("previousIsHome") ?? "false") as boolean;
});
